...
const expressInfo = computed(() => {
  if (!originId.value || !destinationId.value) return null;
  ...
  // 起站非直達車站點，直達車不停靠
  if (!origin.isExpress) return { show: false, annotation: null, expressStationId: null };

  // 迄站是直達車站點，正常顯示
  if (dest.isExpress) return { show: true, annotation: null, expressStationId: null };
  ...
  if (stationGap >= 2) return { show: false, annotation: null, expressStationId: null };
...
const direction = computed<"0" | "1">(() => {
  if (!originId.value || !destinationId.value) return "0";
...
const departureTimetable = computed(() => {
  if (!originId.value || !destinationId.value || originId.value === destinationId.value) return [];
  ...
  return filtered
    .flatMap((item) =>
      (item.Timetables ?? []).map((t) => {
        ...
        // 不該顯示直達車時，過濾掉
        if (isExpress && (!ei || !ei.show)) return null;
        ...
        return {
          ...
          estimatedArrival:
            travelMinutes != null ? addMinutesToTime(t.DepartureTime, travelMinutes) : "—",
...
<template>
  <div>
    <h2 class="text-h5 font-weight-bold mb-4">起訖站查詢</h2>
      ...
      <v-card-text>
        <v-list density="compact">
          ...
          <v-list-item v-if="result.travelTimeNormal != null">
            <template #prepend>
              <v-icon color="blue"> mdi-train </v-icon>
            </template>
            <v-list-item-title>普通車行車時間</v-list-item-title>
            <v-list-item-subtitle> 約 {{ result.travelTimeNormal }} 分鐘 </v-list-item-subtitle>
          </v-list-item>

          <v-list-item v-if="result.travelTimeExpress != null">
            <template #prepend>
              <v-icon color="purple"> mdi-train-variant </v-icon>
            </template>
            <v-list-item-title>
              直達車行車時間
            ...
            <v-list-item-subtitle> 約 {{ result.travelTimeExpress }} 分鐘 </v-list-item-subtitle>
          </v-list-item>

          <v-list-item v-if="result.distance != null">
            <template #prepend>
              <v-icon color="green"> mdi-map-marker-distance </v-icon>
            </template>
            <v-list-item-title>距離</v-list-item-title>
            <v-list-item-subtitle> {{ result.distance }} 公里 </v-list-item-subtitle>
          </v-list-item>
        </v-list>
      </v-card-text>
    </v-card>
    ...
    <v-card
      ...
      variant="outlined"
    >
      <v-card-text class="text-center text-medium-emphasis"> 起站與迄站不可相同 </v-card-text>
    </v-card>

    <v-card v-else-if="!originId || !destinationId" class="mt-4" variant="outlined">
