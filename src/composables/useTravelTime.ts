...
async function fetchData() {
  ...
  error.value = null;
  try {
    const [ttRes, fareRes] = await Promise.all([fetch(TRAVEL_TIME_URL), fetch(FARE_API_URL)]);

    if (!ttRes.ok || !fareRes.ok) throw new Error("API 請求失敗");
...
export function useTravelTime() {
  ...
  function getDistance(origin: string, destination: string): number | null {
    ...
    if (entry) return Number(entry.traveldistance);
