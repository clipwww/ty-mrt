...
async function fetchFirstLastTrain() {
  ...
  try {
    ...
    if (!res.ok) throw new Error("首末班車 API 請求失敗");
    const body: ApiResponse<FirstLastTrainRaw> = await res.json();
    if (!body.success) throw new Error(body.resultMessage || "首末班車 API 回傳失敗");
...
export function useFirstLastTrain() {
  function getStationFirstLastTrain(stationId: string) {
    return firstLastTrainData.value.filter((item) => item.StationID === stationId);
  }
